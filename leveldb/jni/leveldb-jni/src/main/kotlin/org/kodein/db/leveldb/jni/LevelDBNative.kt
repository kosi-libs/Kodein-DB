 ...
 */
class LevelDBNative private constructor(ptr: Long, options: LevelDB.Options) : NativeBound(ptr, "DB", null, options), LevelDB {
    ...
    object Factory : LevelDB.Factory {
        ...
        override fun open(path: String, options: LevelDB.Options): LevelDB {
            ...
            try {
                return LevelDBNative(n_OpenDB(path, optionsPtr, options.repairOnCorruption), options)
            } catch (tr: Throwable) {
                n_ReleaseOptions(optionsPtr)
                if (tr is LevelDBException)
                    throw tr
                throw LevelDBException(tr)
            }

        }

        override fun destroy(path: String) {
            n_DestroyDB(path)
        }
    }
    ...
    override fun put(key: Buffer, value: Buffer, options: LevelDB.WriteOptions) = put(key.toByteBuffer(), value.toByteBuffer(), options)
    ...
    override fun delete(key: Buffer, options: LevelDB.WriteOptions) = delete(key.toByteBuffer(), options)
    ...
    override fun get(key: Buffer, options: LevelDB.ReadOptions) = get(key.toByteBuffer(), options)
    ...
    override fun indirectGet(key: Buffer, options: LevelDB.ReadOptions) = indirectGet(key.toByteBuffer(), options)
    ...
    private class NativeBytes internal constructor(ptr: Long, handler: PlatformCloseable.Handler, options: LevelDB.Options) : NativeBound(ptr, "Value", handler, options), LevelDB.NativeBytes {
        ...
        override val buffer: Buffer = Buffer(n_Buffer(ptr), true)
            get() {
                checkIsOpen()
                return field
            }
    ...
    private class WriteBatch internal constructor(ptr: Long, handler: PlatformCloseable.Handler, options: LevelDB.Options) : NativeBound(ptr, "WriteBatch", handler, options), LevelDB.WriteBatch {
        ...
        override fun put(key: Buffer, value: Buffer) = put(key.toByteBuffer(), value.toByteBuffer())
        ...
        override fun delete(key: Buffer) = delete(key.toByteBuffer())
    ...
    private class Iterator internal constructor(ptr: Long, handler: PlatformCloseable.Handler, options: LevelDB.Options) : NativeBound(ptr, "Iterator", handler, options), LevelDB.Iterator {
        ...
        private class NativeBytesArray(
            ...
            override fun getKey(i: Int): Buffer {
                val index = indexes[i]
                if (index == -1)
                ...
                key.position(keys[i])
                key.limit(values[i])
                return Buffer(key.slice(), true)
            }

            override fun getValue(i: Int): Buffer {
                val index = indexes[i]
                if (index == -1)
                    throw ArrayIndexOutOfBoundsException(i)
                ...
                value.limit(limit[i])
                return Buffer(value.slice(), true)
            }
        ...
        override fun seekTo(target: Buffer) = seekTo(target.toByteBuffer())
        ...
        override fun transientKey(): Buffer {
            return Buffer(n_key(nonZeroPtr()), true)
        }

        override fun transientValue(): Buffer {
            return Buffer(n_value(nonZeroPtr()), true)
        }
    ...
    companion object {
        ...
        @JvmStatic private external fun n_DestroyDB(path: String)
    ...
    override fun noPool(newCapacity: Int): ObjectPool<Buffer> = LevelDB.BufferNoPool(this, newCapacity)

    override fun newBuffer(capacity: Int): Buffer = Buffer(ByteBuffer.allocateDirect(capacity))
