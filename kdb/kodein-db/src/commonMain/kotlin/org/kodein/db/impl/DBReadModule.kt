...
internal interface DBReadModule : DBRead {
    ...
    override fun <M : Any> get(type: TKType<M>, key: Key<M>, vararg options: Options.Read): M? = mdb.get(type, key, *options)?.model

    override fun findAll(vararg options: Options.Read): Cursor<*> = CursorImpl(mdb.findAll(*options))

    override fun <M : Any> find(type: TKType<M>, vararg options: Options.Read): DBRead.FindDsl<M> = FindDslImpl(mdb, type, options)

    override fun getIndexesOf(key: Key<*>, vararg options: Options.Read): Set<String> = mdb.getIndexesOf(key, *options)

    class FindDslImpl<M : Any>(private val mdb: ModelRead, private val type: TKType<M>, private val options: Array<out Options.Read>) : DBRead.FindDsl<M> {
