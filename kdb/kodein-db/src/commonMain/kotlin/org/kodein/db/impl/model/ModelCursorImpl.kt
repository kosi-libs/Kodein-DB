...
internal class ModelCursorImpl<B : Any, M : B>(override val cursor: DataCursor, private val mdb: ModelDBImpl, private val modelType: TKType<M>) : ModelCursor<M>, ResettableCursorModule, Closeable by cursor {
    ...
    override fun model(vararg options: Options.Read): Sized<M> = model ?: mdb.deserialize<M>(modelType.ktype, getDocumentKeyID(key().bytes), cursor.transientValue(), options).also { model = it }
