...
abstract class DataDBTests_07_FindByIndex : DataDBTests() {
    ...
    fun test00_FindByIndexAll() {
        ...
        ddb.findAllByIndex(1, "Symbols").use {
            ...
            assertNotNull(it.transientMetadata())
            assertBytesEquals(array("MetaSymbolsA"), it.transientMetadata()!!)
            it.next()
            assertTrue(it.isValid())
            ...
            assertNull(it.transientMetadata())
            it.next()
    ...
    fun test09_FindByIndexMultiple() {
        ...
        ddb.findByIndex(1, "Symbols", Value.of("alpha")).use {
            ...
            assertBytesEquals(array('i', 0, int(1), "Symbols", 0, "alpha", 0, "beta", 0, "aaa", 0), it.transientSeekKey())
            assertNotNull(it.transientMetadata())
            assertBytesEquals(array("MetaSymbols1"), it.transientMetadata()!!)
            it.next()
            assertFalse(it.isValid())
        }
        ...
        ddb.findAllByIndex(1, "Symbols").use {
            ...
            assertNotNull(it.transientMetadata())
            assertBytesEquals(array("MetaSymbols2"), it.transientMetadata()!!)
            it.next()
            assertFalse(it.isValid())
        }
    }
    ...
    fun test10_FindByIndexWithV0() {
        ...
        ddb.ldb.put(array('i', 0, int(1), "Symbols", 0, "alpha", 0, "beta", 0, "aaa", 0).asMemory(), array(128, ushort(10), ushort(3), "MetaSymbolsA").asMemory())
        ddb.ldb.put(array('i', 0, int(1), "Numbers", 0, "forty", 0, "two", 0, "aaa", 0).asMemory(), array(128, ushort(9), ushort(3)).asMemory())
