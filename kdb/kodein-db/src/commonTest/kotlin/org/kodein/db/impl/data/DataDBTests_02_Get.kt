...
class DataDBTests_02_Get : DataDBTests() {
    ...
    fun test00_GetExisting() {
        val aKey = ddb.newKey(KBuffer.wrap("Test", Charset.ASCII), Value.ofAscii("aaa"))
        ddb.put(aKey, Value.ofAscii("ValueA1!"), indexSet("Symbols" to Value.ofAscii("alpha", "beta")))
        val bKey = ddb.newKey(KBuffer.wrap("Test", Charset.ASCII), Value.ofAscii("bbb"))
    ...
    fun test02_GetUnknownInNonEmptyDB() {
        ...
        assertNull(ddb.get(ddb.newKey(KBuffer.wrap("Test", Charset.ASCII), Value.ofAscii("bbb"))))
    }
