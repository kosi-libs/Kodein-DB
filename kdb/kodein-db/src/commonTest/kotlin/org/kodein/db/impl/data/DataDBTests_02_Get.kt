...
class DataDBTests_02_Get : DataDBTests() {
    ...
    fun test00_GetExisting() {
        val aKey = ddb.putAndGetKey("Test", Value.ofAscii("aaa"), Value.ofAscii("ValueA1!"), indexSet("Symbols" to Value.ofAscii("alpha", "beta"))).key
        ...
        val bKey = ddb.getKey("Test", Value.ofAscii("bbb"))
        assertDataIs(byteArray("ValueB1!"), ddb.get(bKey))
    }

    @Test
    fun test01_GetUnknownInEmptyDB() {
        val key = ddb.getKey("Test", Value.ofAscii("aaa"))
